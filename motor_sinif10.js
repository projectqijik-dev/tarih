...
// =========================================
    let aktifMuzikIndex = -1; 
    window.suAnkiKategoriIndex = -1; 
    let aktifSarkiIndex = -1; 
    let muzikCaliyor = false;
    let gorunumModu = 'kategori'; 
    ...
    function muzikListesiniOlustur() {
        ...
        if (gorunumModu === 'kategori') {
            ...
            geriDiv.style.cursor = 'pointer'; geriDiv.style.padding = '10px 15px'; geriDiv.style.marginBottom = '10px';
            geriDiv.style.borderRadius = '8px'; geriDiv.style.background = 'rgba(255,255,255,0.05)';
            geriDiv.style.display = 'flex'; geriDiv.style.alignItems = 'center'; geriDiv.style.gap = '10px'; geriDiv.style.fontWeight = 'bold';
    ...
    function kategoriSecMuzik(index) {
        aktifMuzikIndex = index; gorunumModu = 'sarki'; muzikListesiniOlustur();
    }
    ...
    function sarkiCal(katIndex, sarkiIndex) {
        ...
        window.suAnkiKategoriIndex = katIndex; aktifMuzikIndex = katIndex; aktifSarkiIndex = sarkiIndex;
        ...
        // --- ASENKRON ÇAKIŞMAYI ÖNLEYEN HARD RESET ---
        ...
        setTimeout(() => {
            ...
            if (playPromise !== undefined) {
                playPromise.then(() => {
                    ...
                    muzikCaliyor = false;
                    document.getElementById('muzikPlayBtn').innerHTML = '<i class="fa-solid fa-play" style="margin-left: 2px;"></i>';
                });
            }
        }, 50);
    }
    ...
    function toggleMuzikPlay() {
        ...
        if (aktifSarkiIndex === -1) { 
            if(typeof window.MUZIK_VERITABANI !== 'undefined' && window.MUZIK_VERITABANI.length > 0 && window.MUZIK_VERITABANI[0].sarkilar.length > 0) {
                sarkiCal(0, 0); 
            }
            return; 
        }
        if (muzikCaliyor) { player.pause(); btn.innerHTML = '<i class="fa-solid fa-play" style="margin-left: 2px;"></i>'; }
        else { player.play(); btn.innerHTML = '<i class="fa-solid fa-pause"></i>'; }
    ...
    function updateMuzikProgress() {
        ...
        if (player && player.duration) { fill.style.width = (player.currentTime / player.duration) * 100 + '%'; }
    }
