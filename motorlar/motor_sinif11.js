// --- 11. SINIF SCRIPT VE API YOLLARI ---
    ...
    const AKTIF_DUYURU_VERSIYONU = "duyuru_v1_11sinif"; 
...
function girisBasariliIslemleri(ogr) {
    ...
    /* --- 3D SANAL MÜZE FONKSİYONLARI --- */
    function muzeGoster() {
        document.getElementById('muzeModal').style.display = 'block';
    }
    
    function muzeKapat() {
        document.getElementById('muzeModal').style.display = 'none';
    }
    
    function muzeOynat(url, baslik) {
        muzeKapat(); // Önce seçim menüsünü kapat
        const container = document.getElementById('inlineMuzeContainer');
        document.getElementById('inlineMuzeBaslik').innerText = baslik;
        document.getElementById('muzeIframe').src = url;
        
        container.classList.remove('hidden');
        // Yumuşak geçiş için
        setTimeout(() => { container.style.opacity = "1"; }, 50);
        
        // Tarayıcı destekliyorsa siteyi gerçek "Tam Ekran" moduna geçir
        if (document.documentElement.requestFullscreen) {
            document.documentElement.requestFullscreen().catch(e => console.log("Tam ekran reddedildi."));
        }
    }
    
    function muzeOynatKapat() {
        const container = document.getElementById('inlineMuzeContainer');
        ...
        setTimeout(() => {
            ...
            document.getElementById('muzeIframe').src = "";
        }, 400); // Animasyonun bitmesini bekle
    }

    /* --- OTOMATİK SINAV ROBOTU JAVASCRIPT FONKSİYONLARI --- */
    ...
    /* --- YEDEKLEME (LOCAL STORAGE) FONKSİYONLARI --- */
